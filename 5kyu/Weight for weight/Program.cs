...
public class WeightSort 
{
  ...
  public static string orderWeight(string strng) 
  {
	...
	*/
    ...
    SortedDictionary<long, List<string>> sortedDictionary = new SortedDictionary<long, List<string>>();
    long sum = 0;
    string res = "";
    
    for (int i = 0; i < nums.Length; i++)
      nums[i] = long.Parse(words[i]);
    ...
    for (int i = 0; i < nums.Length; i++)
    {
      ...
      sum += nums[i];
      nums[i] = sum;
      if (!sortedDictionary.ContainsKey(sum))
      {
        sortedDictionary.Add(sum, new List<string>());
        sortedDictionary[sum].Add(words[i]);
      } 
      else
        sortedDictionary[sum].Add(words[i]);
    }
    
    foreach (KeyValuePair<long, List<string>> kvp in sortedDictionary)
    {
      sortedDictionary[kvp.Key].Sort();
