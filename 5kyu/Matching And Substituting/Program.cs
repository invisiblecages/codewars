...
public class MatchSubst
{
  ...
  {
    var arr = s.Split("\n");  
    var phone = arr[3].Replace("Phone: ", "");
    var ver = arr[5].Replace("Version: ", "");
    
    var phoneNumberPattern = @"^\+1-\d{3}-\d{3}-\d{4}$";
    var versionPattern = @"^\d+\.\d+$";
    if (!Regex.IsMatch(phone, phoneNumberPattern) || !Regex.IsMatch(ver, versionPattern))
      return "ERROR: VERSION or PHONE";
    
    if (double.Parse(ver) != 2.0)
      ver = version;
